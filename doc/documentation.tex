...
\item \texttt{Exact = no}: the metric ($g_{ij}$, lapse $\alpha$, shift
  ...
  polynomial, of which the required derivatives are taken analytically.
...
the target coordinate time is reached.
...
thorn is a unigrid tool: the particle arrays are stored with
\texttt{DISTRIB=CONSTANT} (replicated on every MPI rank) and the
functions are scheduled \texttt{global}, so in a multi-process run the
integration is executed once, on a single rank; the other ranks idle
during the particle step (verified bit-identical against the
single-rank result).  Multi-level (AMR) runs are not supported.
